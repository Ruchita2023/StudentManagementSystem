...
@Generated(value = "org.openapitools.codegen.languages.SpringCodegen", date = "2023-10-06T12:54:41.605736185Z[UTC]")
@Controller
...
public class CourseApiController implements CourseApi {
     ...
     */
    @Override
    public ResponseEntity<Course> updateCourse(@Parameter(name = "courseId", description = "Course id to delete", required = true, in = ParameterIn.PATH) @PathVariable("courseId") Long courseId,@Parameter(name = "Course", description = "Update an existent course", required = true) @Valid @RequestBody Course course
    ){
        CourseDto processedcourse = courseService.updateCourse(courseId, course);  
        return new ResponseEntity<Course>(processedcourse.toEntity(),HttpStatus.OK);
        CourseDto processedcourse = courseService.updateCourse(courseId, course);  
        return new ResponseEntity<Course>(processedcourse.toEntity(),HttpStatus.OK);
    }
